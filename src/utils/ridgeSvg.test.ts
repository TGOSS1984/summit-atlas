...
describe('buildRidgeSvg', () => {
  it('is deterministic for the same mountain id', () => {
    ...
    expect(a.strokePath).toBe(b.strokePath)
  })
  ...
  it('draws different ridges for different ids', () => {
    ...
    expect(a.strokePath).not.toBe(b.strokePath)
  })

  it('closes the fill shape down to the baseline but leaves the stroke open', () => {
    const { fillPath, strokePath } = buildRidgeSvg(makeMountain())
    expect(fillPath.endsWith('Z')).toBe(true)
    expect(strokePath.endsWith('Z')).toBe(false)
  })
  ...
  it('keeps the ridge path inside the viewBox bounds', () => {
    const { strokePath } = buildRidgeSvg(makeMountain({ elevation: 8849 }))
